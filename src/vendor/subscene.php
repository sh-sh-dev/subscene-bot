...
class Subscene {
    ...
    public function __construct($username, $password, $languages = [46], $cookie_file = __DIR__ . '/cookies.txt') {
        // 46 is for Persian
        foreach (['username', 'password', 'cookie_file', 'languages',] as $var) {
            $this->{$var} = ${$var};
        }
    }
    ...
    public function getSubtitles($url, $exit_on_bad_request = false) {
        ...
        $result = [];
        foreach (['title' => '//h2/text()', 'year' => '//li[strong[contains(text(), "Year")]]/text()[last()]', 'poster' => '//img[@alt="Poster"]/@src', 'imdb' => '//a[@class="imdb"]/@href',] as $part => $query) {
    ...
    public function getSubtitleInfo($url, $exit_on_bad_request = false) {
        ...
        foreach (['title' => '//span[@itemprop="name"]', 'poster' => '//img[@alt="Poster"]/@src', 'author' => '//li[@class="author"]/a/text()', 'comment' => '//div[@class="comment"]', 'imdb' => '//a[@class="imdb"]/@href',] as $part => $query) {
            ${$part} = $this->xpathQuery($query, $page);
            if (${$part}->length > 0) {
                $result[$part] = trim(${$part}[0]->nodeValue);
            }
